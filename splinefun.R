...
#peaks and troughs possible to be out of original points
...
n <- 20
set.seed(11)
x. <- seq(1,n,1) ; y. <- cumsum(abs(rnorm(n)))
plot(x., y.)
#curve(splinefun(x., y.)(x), add = TRUE, col = 2, n = 1001)
#curve(splinefun(x., y., method = "natural")(x), add = TRUE, col = 3, n = 1001)
curve(splinefun(x., y., method = "periodic")(x), add = TRUE, col = 4, n = 1001)
#curve(splinefun(x., y., method = "monoH.FC")(x), add = TRUE, col = 5, n = 1001)
curve(splinefun(x., y., method = "hyman")   (x), add = TRUE, col = 6, n = 1001)
legend("topleft",
       paste0("splinefun( \"", c("fmm", "natural", "periodic", "monoH.FC", "hyman"), "\" )"),
       col = 2:6, lty = 1, bty = "n")
